...
$qclient = $localdb->prepare("SELECT * FROM  `analytics` ORDER BY `analytics`.`forge` ASC LIMIT 0, 300000");
$qserver = $localdb->prepare("SELECT * FROM  `analytics` ORDER BY `analytics`.`forge` ASC LIMIT 0, 300000");
...
foreach ($qrowclient as $row) {
	if (array_search(translate($row["forge"]), $qdates) === false) {
		array_push($qdates, translate($row["forge"]));
	}
	$i = array_search(translate($row["forge"]), $qdates);
	$tally["CLIENT"][$i] += 1;
}
...
$graph->title->set("Activations by Forge Build");
